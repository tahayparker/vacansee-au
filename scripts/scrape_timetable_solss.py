...
def fetch_preload(campus_id: str = "1") -> dict:
    r = requests.post(
        PRELOAD_URL, data={"p_campus_id": campus_id, "p_type": "a"}, timeout=10
    )
    r.raise_for_status()
    return r.json()
...
def flatten_normal(
    ...
    for s in subjects:
        ...
        for ti, t in enumerate(s.get("timetable", [])):
            ...
            rooms = (
                [clean_room_fragment(r) for r in split_on_delimiters(room_value)]
                if room_value
                else [""]
            )
...
def scrape(
    raw_dir: Path, session_map: dict[str, str], calendar: AcademicCalendar
) -> list[dict]:
    all_rows: list[dict] = []
    ...
    for campus_id, campus_name in CAMPUSES.items():
        ...
        try:
            ...
            normal_rows, occupied = flatten_normal(
                subjects, campus_name, session_map, calendar
            )
            gsm_rows = flatten_gsm(
                subjects, campus_name, session_map, calendar, occupied
            )
...
def main() -> int:
    ...
    print(
        f"  {len(calendar.standard)} standard session/year entries, "
