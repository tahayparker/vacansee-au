...
export default function SSO() {
      ...
      supabase.auth
        ...
        .then(({ data: { session } }: { data: { session: any } }) => {
          if (session) {
            ...
            setError(
              "No session tokens found in URL. Please go back to the gate and try again.",
            );
          }
        });
    }
  ...
  return (
    <div className="flex h-screen w-full flex-col items-center justify-center gap-4 bg-black z-50 fixed inset-0">
      {error ? (
        <div className="text-red-500 max-w-md text-center p-4 bg-red-500/10 rounded-lg border border-red-500/20">
          {error}
        </div>
      ) : (
